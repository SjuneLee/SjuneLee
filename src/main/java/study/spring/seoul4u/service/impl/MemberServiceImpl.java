...
public class MemberServiceImpl implements MemberService {
	...
	public int selectUserIdEmailCount(Member member) throws Exception {
		...
		try {
			...
			logger.error(e.getLocalizedMessage());
			throw new Exception("아이디, 이메일 일치 검사에 실패했습니다.");
		}
	...
	public Member selectUserIdByTel(Member member) throws Exception {
			Member result = null;
		try {
			result = sqlSession.selectOne("MemberMapper.selectUserIdByTel", member);
			if (result == null) {
				throw new NullPointerException();
			}
		} catch (NullPointerException e) {
			throw new Exception("이름이나 폰번호를 잘못되었습니다.");
		} catch (Exception e) {
			logger.error(e.getLocalizedMessage());
	...
	public Member selectAdminInfo(Member member) throws Exception {
		...
		try {
			result = sqlSession.selectOne("MemberMapper.selectAdminInfo", member);
	...
	public void selectMemberPasswordCount(Member member) throws Exception {
		try {
			...
			// 회원번호와 아이디, 비밀번호가 일치하는 데이터가 0이라면, 비밀번호가 잘못된 상태
			if (result == 0) {
				throw new NullPointerException();
			}
		} catch (NullPointerException e) {
			throw new Exception("아이디나 비번이 잘못되었습니다.");
		} catch (Exception e) {
			logger.error(e.getLocalizedMessage());
			throw new Exception("아이디, 비번 검사에 실패했습니다.");
		}
	}
	...
	public void deleteMemberByAdmin(Member member) throws Exception {
		try {
			int result = sqlSession.delete("MemberMapper.deleteMemberByAdmin", member);
			// 삭제된 데이터가 없다는 것은 WHERE절의 조건값이 맞지 않다는 의미.
			// 이 경우, 첫 번째 WHERE조건에서 사용되는 id값에 대한 회원을 찾을 수 없다는 의미
			if (result == 0) {
				throw new NullPointerException();
			...
			//sqlSession.rollback();
			logger.error(e.getLocalizedMessage());
			throw new Exception("회원삭제에 실패했습니다.");
		}
